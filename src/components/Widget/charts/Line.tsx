...
import { renderLine } from 'utils/charts'
...
const LineSettings: FC<LineSettingsProps> = ({ id, columnNames, settings, isPublished }) => {
...
const Line: FC<LineProps> = ({ id, collectionId, timeColumnId, valueColumnId, xLabel, yLabel, color, getContentCallback }) => {
  ...
  useEffect(() => {
    ...
    if (data && node) {
      const svg = renderLine(data, timeColumnId, valueColumnId, xLabel, yLabel, color)
      node.append(svg)
